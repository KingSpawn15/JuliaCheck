...
function calculate_gaussian_laser_spot(XPRIME::Array{Float64}, ZPRIME::Array{Float64},
    ...
    gaussian_laser_spot[:,:,abs.(vec(zprime)) .> z_max,:] .= 0;
...
# %   Detailed explanation goes here
    ...
    # threaded_calculation!(40, interaction_v, discretization.xprime, discretization.yprime, discretization.zprime, 
    ...
    # Threads.@threads for time_ind in 1:l_tc

    #     if(mod(time_ind,10)==0)
    #         print("$(time_ind) out of $(l_tc) \n")
    #     end

    #     # interaction_v[time_ind,:] .= (1/(4*pi*EPSILON_0)).*trapz((discretization.xprime,discretization.yprime,discretization.zprime,:),
    #     # calculate_g2(omega_y, t_c_subsampled[time_ind] .- t_r, phase, gamma, gamma_factor).*YPP_GK_g1)

    #     integrate_v!(interaction_v, time_ind, discretization.xprime,discretization.yprime,discretization.zprime,
    #     calculate_g2(omega_y, t_c_subsampled[time_ind] .- t_r, phase, gamma, gamma_factor).*YPP_GK_g1)
