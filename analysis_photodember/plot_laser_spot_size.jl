...
sub_base = "laser_spot_fwhm"
base = base*sub_base

# array_laser_spot_fwhm = laser_spot_fwhm_o.*10.0.^(range(-3,stop=-2.5,length=3))
...
psi_sub_array = zeros(length(t_w),length(e_w),length(array_laser_spot_fwhm));
...
titlep = plot(title= " Laser Spot Size Dependence", grid = false, showaxis = false, bottom_margin = -50Plots.px)
l = @layout [A{0.01h}; (length(array_laser_spot_fwhm),6)]
plot(titlep,plot_agg_incoherent...,plot_agg_sub...,layout=l)
savefig("./analysis_photodember/saved-plots/"*sub_base*".svg")
