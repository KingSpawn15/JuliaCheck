...
psi_sub_array = zeros(length(t_w),length(e_w),length(array_pulse_energy_experiment));
...
titlep = plot(title= " Laser Energy Dependence", grid = false, showaxis = false, bottom_margin = -50Plots.px)
l = @layout [A{0.01h}; (length(array_pulse_energy_experiment),6)]
