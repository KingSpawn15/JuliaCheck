...
base="analysis_photodember/saved-matrices/materialanalysis_withoutcutoff"
...
function generate_matrices(dict:: Dict{String, Vector{Float64}}, 
    ...
    for field in keys(dict)
        ...
        for (ind,val) in enumerate(dict[field])
            ...
            @time _, _, interaction_v_pd=mod_cdem.interaction_potential_photodember(dis_sp, mat, las, numericalp; cutOff = -10.0)
